	...
	if len(isoListX)>1:
		...
		for i in xrange(len(isoListX)-1):#iterate through all but the last item in the list in this case the first isotope
			...
			for ind in targetInd:
				...
				if j==len(nextIsoY):#catch errors
					j-=1
...
def processSpectrum(X, Y, scales, minSNR, pkResEst, corrCutOff,
				    maxCharge = 1, xDiff = None, groupPeaks = False,
				    xTol = 500):
	'''
...
#			print peakLoc
			...
			if groupPeaks:
				returnANS = sortPeaks(centX, centY, isoX, isoY, corrFactors, centSNR, xTol)
			else:
...
#	stop = 22000
	...
	t1 = time.clock()
	grpPeaks = True
	smoothY = smoothF.savitzky_golay(abund)
	ANS, boolAns = processSpectrum(mz, smoothY, scales, minSNR, resEst, xDiff = mzDiff, corrCutOff = corrCutOff, groupPeaks = grpPeaks)
	print "Peak Picking and Isotopic Fitting Time: ", time.clock()-t1

	fig = P.figure()
	ax = fig.add_subplot(111)
#	ax.plot(mz, abund, 'b', alpha = 0.6)
	ax.plot(mz, smoothY, 'g', alpha = 0.4)
