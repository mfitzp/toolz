...
pyXMLOK = False
try:
    from xml.dom.ext import PrettyPrint#Needs PyXML installed
    pyXMLOK = True
except:
    print "PyXML Needs to be installed" 

import zlib
import gzip

import numpy
from numpyXML import encodeArray, decodeArray
...
class Object2Xml:
    ...
    def print_xml(self,filename,**kwargs):
        f = open(filename,'w')
        if self.xmlobj is not None:
            #~ self.xmlobj.writexml(f)
            f.write(self.xmlobj.toxml())
            #~ f.write(self.xmlobj.toprettyxml())
            #~ PrettyPrint(self.xmlobj, f)
        f.close()
...
#        
            ...
            #print objname, obj.__class__
            return None
    
        #test for numpy BHC
        numpyArrayBool = False
        if 'numpy.ndarray' in _xmltype:
            numpyArrayBool = True
            #~ print objname
            #~ print _xmltype
            #~ print _xmlid
        ...
        if id(obj) in self._vars:
            #print _xmltype, objname, _xmlid, type(_xmltype)
            if _xmltype == 'float':
                #print "FLOAT", _xmltype, objname, _xmlid, type(_xmltype)
                return self._create_element(objname, _xmlid, _xmltype, str(obj.__float__()))
            elif _xmltype == 'int':
                #print "INT", _xmltype, objname, _xmlid, type(_xmltype)
                return self._create_element(objname, _xmlid, _xmltype, str(obj.__int__()))
            elif _xmltype == 'str':
                #print "INT", _xmltype, objname, _xmlid, type(_xmltype)
                return self._create_element(objname, _xmlid, _xmltype, str(obj.__str__()))
            else:
        ...
        if self._adapt.has_key(obj.__class__):
            ...
            if obj is None:
                ...
                if numpyArrayBool:#BHC added if statement
                    #print "Numpy Bool True"
                    #print obj.__repr__()
                    validTypes = ['_xy', '_offsets', '_sizes', '_paths']
                    if objname in validTypes:#'_xy' or objname == '_offsets' or objname == '_sizes' or :
                        if objname == '_paths':
                            print "PATHS", obj.__repr__()
                            
                        encodedData = encodeArray(obj.ravel())
                        o = encodedData.__repr__()
                    else:
                        o = obj.__repr__()
                else:
                    o = obj.__repr__()
