...
class RectangleSelector:
    ...
    def __init__(self, ax, onselect, drawtype='box',
        ...
        Added BHC -- the primaryBtn is used to focus on a single button if desired
    ...
    def ignore(self, event):
        ...
        if not self.active:
            return True
        ##############Added by BHC##################
        # if the primary button was not used

        if self.primaryBtn != None:
            if event.button != self.primaryBtn:
                return True
        ##############End BHC Addition##################
