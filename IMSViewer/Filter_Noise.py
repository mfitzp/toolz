...
def Filter_Raw_Spectrum():
    #file_name = FP.File_Dialog()
    #print file_name
    #data_array=FFT.get_ascii_data(file_name)#setup for csv
    #data_array=FFT.get_ascii_data('/home/clowers/Desktop/IMS-TOF Noise Project/Noisy_IMS_XY.csv')
    #data_array=FFT.get_ascii_data('C:\\Documents and Settings\\d3p483\\Desktop\\IMS-TOF Noise Project\\BSA_XY_Full.csv')
    data_array=FFT.get_ascii_data('Noisy_IMS_XY.csv')
    ...
    fft_output = FBH.remove_peaks_by_harmonic(fft_output)
    ...
    #filtered_spectrum = fftpack.ifft(filtered_dict.get('fft_result'))
    #filtered_spectrum = FBH.transform_segment(interp_list[1])
    #summary_plot(interp_list, filtered_dict, peak_results, filtered_spectrum)
    summary_plot(interp_list, fft_output, peak_results, filtered_spectrum)
    #comparison_plot(interp_list, filtered_spectrum)
