...
import numpy
from numpyXML import encodeArray, decodeArray#BHC
...
class XMLFigure(ContentHandler):
    ...
    Added support for numpy arrays, mpl text instances and CircleCollections
    Also added support for compressed xml files...through addition of parseString methods
    
    
    vlines are also not supported right now
    """
    
    def create_parser(self, parseStr = False):
        parser = make_parser()
        parser.setContentHandler(self)
        return parser         
    
    def __init__(self, xmlfile, parseStr = False, standAlone = True):
        
        #xmlfig = zlib.decompress(xmlfig)
        if '.gz' in xmlfile:
            f = gzip.open(xmlfile, 'rb')
            xmlfile = f.read()
            f.close()
            parseStr = True
            
        if parseStr:
            self._dom = xml.dom.minidom.parseString(xmlfile)
        else:
        ...
        #Not exactly sure why this is necessary but it is critical
        self.parser = self.create_parser()       
        if parseStr:
            parseString(xmlfile, self)
        else:
    ...
    def setAxLists(self):
        self.axlist = self._get_node_byname('axes',self._xmlfig)
        self.axlist = self.axlist._get_childNodes()        
    ...
    def _update_axes(self, xmlAxes, mplAx):    
        _xmlaxes = xmlAxes
        kwargs = {}
        ...
        title = self._get_value('title._text',_xmlaxes)
        kwargs['title'] = title
        
        #font size is at xaxis.offsetText._fontproperties._size            
        _points = self._get_value('viewLim._points', _xmlaxes)
        #print _points
        #kwargs['xlim'] = _points[:,0]
        #kwargs['ylim'] = _points[:,1]
                   
        mplAx.set_xlim(_points[:,0])
        mplAx.set_ylim(_points[:,1])
        
        mplAx.set_title(kwargs['title'])
        mplAx.set_xlabel(kwargs['xlabel'])
        mplAx.set_ylabel(kwargs['ylabel'])
        #~ print "GO JOE"
        #print kwargs['title'], kwargs['xlabel'], kwargs['ylabel']        
...
#                'yscale': None,
            ...
            self._add_collecions(_xmlaxes, axes)
            self._add_texts(_xmlaxes, axes)            
    ...
    def _add_texts(self, _xmlaxes, mplAx):
        #BHC Added 
        txtList = self._get_node_byname('texts',_xmlaxes)
        txtList = txtList._get_childNodes()            
        for _txt in txtList:
            kwargs = {
                       'text':'',
                       'x':None,
            ...
            _text = self._get_value('_text',_txt)
            kwargs['text'] = _text

            _x = self._get_value('_x',_txt)
            kwargs['x'] = _x

            _y = self._get_value('_y',_txt)
            kwargs['y'] = _y
            
            _color = self._get_value('_color',_txt)
            kwargs['color'] = _color

            _alpha = self._get_value('_alpha',_txt)
            kwargs['alpha'] = _alpha
          
            _rotation = self._get_value('_rotation',_txt)
            kwargs['rotation'] = _rotation                        

            
            _fontMgr = self._get_node_byname('_fontproperties', _txt)
            _fontsize = self._get_value('_size',_fontMgr)
            kwargs['fontsize'] = _fontsize
            
            mplAx.text(kwargs['x'], kwargs['y'], kwargs['text'], color = kwargs['color'],\
                       alpha = kwargs['alpha'], fontsize = kwargs['fontsize'], rotation = kwargs['rotation'])
            

    def _add_collecions(self, _xmlaxes, axes):
        #~ print "Add Collections"
        from matplotlib.collections import CircleCollection#Scatter
        from matplotlib.collections import LineCollection#Vlines
        
        collectionList = self._get_node_byname('collections', _xmlaxes)
        collectionList = collectionList._get_childNodes()
        #print collectionList
        circleBool = False
        lineBool = False
        for _xmlcollection in collectionList:
            if _xmlaxes.getAttribute('type') == 'matplotlib.collections.LineCollection':
                lineBool = True
                circleBool = False
            elif _xmlaxes.getAttribute('type') == 'matplotlib.collections.CircleCollection':
                    lineBool = False
                    circleBool = True
            ...
            #~ kwargs['color'] = self._get_value('_color',_xmlcollection)
            #~ kwargs['marker'] = self._get_value('_marker',_xmlcollection)
            #~ kwargs['markersize'] = self._get_value('_markersize',_xmlcollection)
            kwargs['edgecolors'] = self._get_value('_edgecolors',_xmlcollection)
            kwargs['facecolors'] = self._get_value('_facecolors',_xmlcollection)
            kwargs['label'] = self._get_value('_label',_xmlcollection)
            kwargs['alpha'] = self._get_value('_alpha',_xmlcollection)
            
            _offsets = self._get_node_byname('_offsets', _xmlcollection)
            #print _offsets
            _offsets = self._get_value('_offsets',_xmlcollection)
            _offsets = decodeArray(_offsets)#BHC
            
            _sizes = self._get_node_byname('_sizes', _xmlcollection)
            #print _sizes
            _sizes = self._get_value('_sizes',_xmlcollection)
            _sizes = decodeArray(_sizes)#BHC                
            ...
            sizeOK = False
            if _sizes != None:
                if len(_sizes) == len(_x):
                    sizeOK = True
                
            x_isdata = self._get_value('x_isdata',_xmlcollection)
            y_isdata = self._get_value('y_isdata',_xmlcollection)
        
            if x_isdata and y_isdata:
                #print kwargs
                if sizeOK:
                    axes.scatter(_x,_y,_sizes,**kwargs)
                else:
                    axes.scatter(_x,_y,**kwargs)
...
#                      'markevery': None,
            ...
            kwargs['markersize'] = self._get_value('_markersize',_xmlline)
            kwargs['markerfacecolor'] = self._get_value('_markerfacecolor',_xmlline)
            kwargs['linestyle'] = self._get_value('_linestyle',_xmlline)
            kwargs['label'] = self._get_value('_label',_xmlline)
            kwargs['alpha'] = self._get_value('_alpha',_xmlline)
            ...
            if _xy.getAttribute('type') == 'numpy.ma.core.MaskedArray':
                ...
                #print "mask = ",mask
                data = decodeArray(data)#BHC
                mask = decodeArray(mask)#BHC
