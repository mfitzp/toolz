...
#------------------------------------------------------------------------------
from scipy import array, arange, sqrt, argmax, zeros, nonzero, take
...
def _decimate(x, y, keep, si, ei, tolerance):
    ...
    top = abs( xei_minux_xsi * (y[si] - y0) - (x[si] - x0) * yei_minux_ysi )
    # The algorithm currently does an expensive sqrt operation which is not strictly
    # necessary except that it makes the tolerance correspond to a real world
    # quantity.
...
if __name__ == "__main__":
    ...
    from cp.io.las_well import LAS
    las = LAS(r"../../../apps/proava2/data/sample.las")
    log_suite = las.get_log_suite()
    print log_suite
    vp = log_suite.get_log('CALI')

    nx,ny = decimate(log_suite.reference.values,vp.values,0.05)

    print 'before ', len(vp.values),'after ', len(nx)

    import gui_thread
    gui_thread.start()

    xplt.plot(log_suite.reference.values,vp.values)
    xplt.hold('on')
    xplt.plot(nx,ny,'k')
    xplt.hold('off')
    print 'hit return to finish'
    # keep plot displayed until users hits return ....
    line = raw_input()
