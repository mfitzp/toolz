...
functions = {
	...
	"Exponential Decay" : fit_expdecay_dic,
	"Exponential Grow" : fit_expgrow_dic,
	"Exponential Assoc" : fit_expassoc_dic,
	"Hyperbolic" : fit_hyperbl_dic,
	"Pulse" : fit_pulse_dic,
	"Rational0" : fit_rational0_dic,
	"Sine" : fit_sine_dic,
	"Gauss Amp" : fit_gaussamp_dic,
	"Allometric" : fit_allometric_dic
}


def fitting(fn,data,parms=None,steps=None):
    """ fitting(fn,data,parms=None,steps=None) - This function uses leastSquaresFit to calculate the fitting
    ...
    parms - returns the fitted parmeters errs  - fitting least square  error returned
    """
    try:
        f = functions[fn]
        np = f['NumPar']
...
def plotfitting(f,x,y1,y2,ret,legd=None):
    """
    plotfitting(f,x,y1,y2,ret,legd=None) - this function use pylab plot routine to do fitted plot
...
def setlegend(self):
	...
	if self.legd == 1:
	    ...
	    for i in range(len(parms)):text(0,ymax-1.5*dy-i*.5*dy,str(parms[i]))
...
def polyfitdialog(root,data=None,title=None):
    ...
    if data == None:
        data = [ (0., 0.), (1., 1.1), (2., 1.98), (3., 3.05), (4.,2.) ]
    if data != None:
        if str(type(data[0])) != "<type 'turple'>":
            data = fitdata(data)
        Fit.data = data
        x=[]
        y=[]
...
if __name__ == '__main__':
    ...
    import pylab as P
    import scipy as S
    x = N.arange(0, 100, 0.1)
    xtra = 15*S.rand(len(x))

    P.subplot(211)

    y = x+xtra
    P.plot(x, y, 'o', label = 'linear')
...
#        return(a *(x-x0)**2 + b*(x-x0) + c )
    ...
    b = 0.02
    c = 0
    quad = a *(y-x0)**2 + b*(y-x0) + c
    P.plot(x, quad, 'o',  label = 'quad')
...
#        return(a * exp(-2*(x-x0)**2/w**2) + y0)
    ...
    gauss = a * exp(-(x-x0)**2/w**2) + y0+xtra
    P.plot(x, gauss,  label = 'gauss')
...
#        return( a * w **2 / ((x-x0)**2 + w**2) )
    ...
    x0 = 60
    w = 5
    lz = a * w **2 / ((x-x0)**2 + w**2)+xtra
    P.plot(x, lz,  label = 'lorentz')
...
#        return((a1 - a2)/(1 + exp((x - x0)/dx)) + a2)
    ...
    a2 = 150
    dx = 1
    boltz = (a1 - a2)/(1 + exp((x - x0)/dx)) + a2 + xtra
    P.plot(x, boltz, label = 'boltz')
...
#        return((a1 - a2)/(1 + (x/x0)**p) + a2)
    ...
    a2 = 50
    p = 3
    logistic = (a1 - a2)/(1 + (x/x0)**p) + a2+xtra
    P.plot(x, logistic,  label = 'logistic')
#
#    def fit_expdecay(parameters, values):
#        x0, y0, a, t = parameters
#        x = values
#        return(y0 + a * exp(-(x - x0)/t))
    ...
    exdecay = y0 + a * exp(-(x - x0)/t)+xtra
    P.plot(x, exdecay, label ='exdecay')
...
#        return(y0 + a * exp((x - x0)/t))
    ...
    a=120
    t=25
    exgrow = y0 + a * exp((x - x0)/t)+xtra
    P.plot(x, exgrow,  label = 'exgrow')
#    def fit_expassoc(parameters, values):
#        y0, a1, t1, a2, t2 = parameters
#        x = values
#        return(y0 + a1 * (1 + exp(-x/t1)) + a2 * (1 + exp(-x/t2)))
    ...
    a2 = 50
    t2 = 2
    exAssoc=y0 + a1 * (1 + exp(-x/t1)) + a2 * (1 + exp(-x/t2)) +xtra
    P.plot(x, exAssoc,  label ='exAssoc')
...
#        return(p1 * x/ (p2 + x))
    p1 = 60
    p2 = 88
    hyperb = p1*x/(p2+x) +xtra
    P.plot(x, hyperb,  label = 'hyperb')
#
#    def fit_pulse(parameters, values):
#        x0, y0, a, t1, t2 = parameters
#        x = values
#        return(y0 + a * (1 + exp(-(x - x0)/t1)) * exp(-(x - x0)/t2))
    ...
    t1=3
    t2=3
    pulse = y0 + a * (1 + exp(-(x - x0)/t1)) * exp(-(x - x0)/t2)+xtra
    P.plot(x, pulse,  label = 'pulse')
...
#        return(a * sin(pi*(x - x0)/w))
    ...
    a=50
    w=10
    siny = a * N.sin(pi*(x - x0)/w)+xtra+y0
    P.plot(x, siny, label='sin')
#
#    def fit_gaussamp(parameters, values):
#        x = values
#        a, x0, w, y0 = parameters
...
#    #	return(a0 * exp(-(x-a1)**2/a2**2/2) *( a3*x**2 + a4 * x + a5 ))
    ...
    w = 10
    y0 = 0
    gaussAmp = a * exp(-(x - x0)**2/(2*w**2)) + y0+xtra
    P.plot(x, gaussAmp,  label='gaussAmp')
#    def fit_allometric(parameters, values):
#        a, x0, c = parameters
#        x = values
#        return(a * (x-x0)**2 + c)
    ...
    x0 = 0
    c = 0
    allo = a * (x-x0)**2 + c+xtra
    #print N.min(allo),  N.max(allo)
    P.plot(x, allo, label='allo')
