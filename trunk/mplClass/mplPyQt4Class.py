...
def main():
    from mplXML import xml2fig
    app = QtGui.QApplication(sys.argv)
    nf = xml2fig.XMLFigure('test3.xml.gz', standAlone = False)
    w = MPL_WIDGET(enableAutoScale = True, enableEdit = True, fig = None)
    x = N.arange(0, 20, 0.1)
    y = N.sin(x)
    y2 = N.cos(x)
    ...
    #~ for i in xrange(200):
        #~ nf._add_texts(nfMPLAX, curAx)
        #~ nf._add_collecions(nfMPLAX, curAx)
        #~ nf._add_lines(nfMPLAX, curAx)
        #~ curAx.cla()
