...
#        self.setAttribute(Qt.WA_DeleteOnClose)
    ...
    def _initConnections_(self):
        self.connect(self.tableWidget, SIGNAL("itemChanged(QTableWidgetItem*)"), self.updateItem)
#        self.connect(self, SIGNAL("clicked(QModelIndex)"), self.updateItem)

    def updateItem(self, tableItem):
        ...
        if data != None:
            ...
            if type(data) is N.ndarray:
                for row in data:
                    ...
                    for item in row:
                        #print repr(str(item))
                        newitem = QTableWidgetItem('%.4f'%item)#'%.3f'%item)
#                        newitem = QTableWidgetItem(QVariant(item))#'%.3f'%item)
#                        print newitem.type()
...
#                    m+=1
    ...
    def __initActions__(self):
        ...
        self.testAction = QAction("Get Help",  self)
