...
class DoubleMyMplCanvas(FigureCanvas):
    def __init__(self, parent=None, width = 6, height = 5, dpi = 100, sharex = None, sharey = None, shareAxis = False):
        self.fig = Figure(figsize = (width, height), dpi=dpi, facecolor = '#FFFFFF')
        self.ax = self.fig.add_subplot(211, sharex = None, sharey = None)
        if shareAxis:
            self.ax2 = self.fig.add_subplot(212, sharex = self.ax, sharey = None)
        else:
            self.ax2 = self.fig.add_subplot(212, sharex = None, sharey = None)
...
class MPL_Widget(QtGui.QWidget):
    def __init__(self, parent = None, enableAutoScale = False, enableCSV = False, enableEdit = False, doublePlot = False, layoutDir = 'h', shareAxis = False):
        QtGui.QWidget.__init__(self, parent)
        if doublePlot:
            self.canvas = DoubleMyMplCanvas(shareAxis = shareAxis)
        else:
...
#            self.layout.addWidget(self.toolbar)
        ...
        #FIX THIS
        if enableAutoScale:
            print "Enable AutoScale has been disabled."
