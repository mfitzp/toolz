...
from aminoacids import aaDict, COLORS
...
class DiffTable(QWidget):
    def __init__(self, dataList = None, colHeaderList = None, rowHeaderList = None, enableSort = False, title = None):
...
#        self.tableWidget.addData(self.data, enableSort = self.sortOk)
        if type(colHeaderList) == list:
            ...
            for i in xrange(len(colHeaderList)):
...
#        tw_menu.addAction(self.testAction)
    ...
    def addData(self, data, startrow=None,  startcol = None, enableSort = False):
        ...
        for m,row in enumerate(data):
            n = sc
            zeroFound = False#for a difference matrix, want to ignore half
            for n,item in enumerate(row):
                if item == 0:
                    zeroFound = True
                if not zeroFound:
                    newitem = QTableWidgetItem(str(item))
                    #used to highlight specific differences between a list and actual data
                    ans = matchDiffVal(item, aaDict, 500)
                    if ans[0] == 1:#this 500 is hardcoded need to variablize...
                        tempColorStr = COLORS[0]
                        newitem.setBackgroundColor(QColor(COLORS[m%len(COLORS)]))
                        newitem.setToolTip(ans[1])
                    elif ans[0] == 2:
                        newitem.setBackgroundColor(QColor('#F5F5F5'))#WhiteSmoke
                else:
                    newitem = QTableWidgetItem("")
                    newitem.setBackgroundColor(QColor('#848484'))#Smoke
...
def matchDiffVal(diffVal, valDict, tol):
    '''
    compares diffVal to the values in valDict based upon the tolerance (tol)
    Originally designed to compare mass differences in spectra to a list of amino acids
    tol is in ppm or a percentage difference
    '''
    compVals = N.array(valDict.keys())
    compVals.sort()

    for val in compVals:
        diff = N.abs(diffVal)
        expTol = val*(tol/1E6)#convert to ppm
        if diff == 0.0:
            return 2, ""
        if diff<=expTol:
            label = valDict[val]["abbr"]
            return 1, label
...
def main():
    ...
    a = S.rand(20)
    a+=128#simulate AA
    colHeaderList = N.array(a, dtype = N.str).tolist()
    rowHeaderList = N.array(a, dtype = N.str).tolist()

    ans = calc_dist_matrix(a,a)
    print ans
    w = DiffTable(dataList = ans, colHeaderList = colHeaderList, rowHeaderList = rowHeaderList)
    #w.show()
    sys.exit(app.exec_())
